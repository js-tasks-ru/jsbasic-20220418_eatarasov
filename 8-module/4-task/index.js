...
export default class Cart {
  ...
  renderModal() {
    ...
    this.cartItems.map((item) => {
      return cart.append(this.renderProduct(item.product, item.count));
    })
    cart.append(this.renderOrderForm());

    this.modal.setBody(cart);
    let buttonsMinus = Array.from(cart.querySelectorAll(".cart-counter__button_minus"));
    let buttonsPlus = Array.from(cart.querySelectorAll(".cart-counter__button_plus"));

    // buttonsMinus.map((button) => {
    //   button.onclick = ({ target }) => {
    //     let productId = target.closest("div[data-product-id]").getAttribute('data-product-id');
    //     this.updateProductCount(productId, -1);
    //   }
    // });

    // buttonsPlus.map((button) => {
    //   button.addEventListener("click", (event) => {
    //     let productId = event.target.closest("div[data-product-id]").getAttribute('data-product-id');
    //     this.onProductUpdate(productId, 1);
    //   });
    // });
    ...
    let formElement = cart.querySelector(".cart-form");
  ...
  onSubmit(event) {
    ...
    let btnSubmit = document.body.querySelector(".cart-buttons__button");
